...
class Operation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "new_experience": self.new_experience.to_dict() if self.new_experience else None,
            "reason": self.reason
        }
...
class ExperienceManager:
    ...
    def __init__(self, llm_config: LLMConfig, pool_path: Optional[str] = None, max_pool_size: int = 100):
        self.llm_client = LLMClient(llm_config)
        self.pool = ExperiencePool(pool_path)
        self.max_pool_size = max_pool_size
    
    def merge_experiences(self, new_experiences: List[Experience], batch_id: Optional[str] = None) -> Dict[str, Any]:
        """将新经验合并到经验池"""
        if batch_id is None:
        ...
        if len(self.pool) == 0:
            ...
            result = {
                ...
                "summary": f"初始化经验池，插入 {len(new_experiences)} 条经验"
            }
        ...
        prompt = EXPERIENCE_MERGE_PROMPT.format(
            ...
            new_experiences=json.dumps(
                [exp.to_dict() for exp in new_experiences],
                ensure_ascii=False,
                indent=2
            )
        )
    ...
    def _execute_operation(self, operation: Operation, batch_id: str):
        ...
        if operation.action == OperationType.INSERT:
            ...
            if operation.new_experience:
                self.pool.insert(operation.new_experience)
                print(f"[REPLACE] 替换 {operation.target_ids} -> {operation.new_experience.id}")
            ...
            if operation.new_experience:
                self.pool.insert(operation.new_experience)
                print(f"[MERGE] 合并 {operation.target_ids} -> {operation.new_experience.id}")
        
        elif operation.action == OperationType.SKIP:
            print(f"[SKIP] 跳过: {operation.reason}")
    ...
    def get_experiences_for_prompt(self, max_count: int = 20) -> str:
        ...
        for i, exp in enumerate(selected, 1):
            ...
            output_lines.append(f"   解决方案: {exp.solution}")
            if exp.code_pattern:
                output_lines.append(f"   代码示例: {exp.code_pattern[:200]}")
            output_lines.append("")
