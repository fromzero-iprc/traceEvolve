...
def main():
    ...
    # 检查 API 配置
    if not args.api_key:
        print("警告: 未设置 LLM API Key，请通过 --api-key 参数或 LLM_API_KEY 环境变量设置")
        print("      将使用模拟模式进行演示")
    
    # 处理日志文件
    results = pipeline.process_log_files(log_files, batch_size=args.batch_size)
